<html><head>
    ...
    // Use an IP geolocation service to get the time zone information
    try {
        $timezoneData = file_get_contents("https://ipinfo.io/{$ip}/timezone/");
        $timezone = trim($timezoneData);

        // Create a DateTime object for the current time in the visitor's time zone
        $currentTime = new DateTime('now', new DateTimeZone($timezone));

        // Get the offset from Zulu time (UTC)
        $offset = $currentTime->getOffset() / 3600;
    } catch (Exception $e) {
        // Handle the exception here, e.g. log the error or display a fallback time zone
        $timezone = 'UTC';
        $offset = 0;
    }
