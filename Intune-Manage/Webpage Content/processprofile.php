...
if (isset($_POST['password'], $_POST['cpassword'], $_POST['email'])) {
	...
	if (empty($msg)) {
		...
		if ($stmt->num_rows > 0) {
            ...
            $stmt = $con->prepare('INSERT IGNORE INTO accounts (password, email, activation_code, role, registered, last_seen, ip, golden, outdated, primaryid, regtype, canbackup, canrestore, canviewlogs, canmanagebackups, cancheckdrift, canmanagedrift, canmanagetenants, cangolddeploy, canviewreports, candeployapps, candeploytemplates, canmigrate, reponame, canmanageapi) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)');
			// We do not want to expose passwords in our database, so hash the password and use password_verify when a user logs in.
			$password = !empty($_POST['password']) ? password_hash($_POST['password'], PASSWORD_DEFAULT) : $password;
			$paid = "paid";
			$stmt->bind_param('ssssssssiisiiiiiiiiiiiisi', $password, $_POST['email'], $uniqid, $role, $date, $date, $ip, $_POST['golden'], $_POST['daystocheck'], $primaryid, $paid, $pcanbackup, $pcanrestore, $pcanviewlogs, $pcanmanagebackups, $pcancheckdrift, $pcanmanagedrift, $pcanmanagetenants, $pcangolddeploy, $pcanviewreports, $pcandeployapps, $pcandeploytemplates, $pcanmigrate, $reponame, $pcanmanageapi);
			$stmt->execute();
			$stmt->close();
			
						// Write to auditlog with userID, IP address, timestamp and update message
						$adminemail = $_POST['email'];
						$auditlog_message = "Customer added: " . $adminemail;
			...
			// Update the session variables
				...
				header('Location: profile-select.php?updatemessage=Customer Added');
				exit;
